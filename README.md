...
| `BACKUP_REMOTE_PATH`  | Optional remote destination for a post-backup sync — use `s3:<bucket>/<prefix>` for S3 | _(empty)_ |
| `BACKUP_REMOTE_SYNC_CMD` | Sync command run as `<cmd> <BACKUP_PATH> <BACKUP_REMOTE_PATH>` | `rsync -az` |
| `BACKUP_REMOTE_FETCH_CMD` | Non-destructive copy command used by `make backup-restore ... REMOTE=1` | `rclone copy` |
| `RCLONE_CONFIG_S3_*`  | rclone remote `s3` credentials/config (type, provider, access key, secret, region, endpoint) for S3 / S3-compatible storage | _(empty)_ |
...
### Remote storage — S3 and S3-compatible
