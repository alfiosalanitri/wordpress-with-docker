...
│       └── deploy.yml           # Deploy via FTP on push to main
...
3. Creates the `public_html/` and `logs/` directories
4. Downloads the latest version of WordPress into `public_html/`
5. Asks whether to build and start the containers
...
├── public_html/
├── my_website.sql           # dump generated by make db-backup-repo
└── .gitignore
```
